<script lang="ts">
    ...
    let fractalColor : RgbaColor = {r: 229, g:204, b:153, a: 1}
    let dimensions = 2.;
    let mandelBoxIterations = 10;
    ...
    if (param) {
        param.onInitialize = ()=>{
            ...
            param.changeValue(renderer.device, Number(mandelBoxIterations), 1);
...
<h1 class="text-2xl py-4 font-bold">Mandelbox Parameters</h1>
<div class="flex flex-col">
    <ColorPicker label="Fractal Color" bind:rgb={fractalColor}/>
</div>
<Slider name="Dimension"             bind:number={dimensions}           min=1.5     max=5   step=0.05    />
<Slider name="Mandelbox Iterations" bind:number={mandelBoxIterations} min=1     max=30   step=1       />
