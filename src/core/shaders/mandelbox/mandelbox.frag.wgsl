...
fn main(
    ...
    // var ro = vec3(0.0, 0.0, 0.0);
    
    let sundir = normalize(vec3<f32>(0.1, 0.8, 0.6)); 
    let sun = vec3<f32>(1.64, 1.27, 0.99); 

    var forward = normalize(vec3<f32>(0.0) - ro);
    // var forward = normalize(camera.posOffset.xyz - ro); 
    // var forward = normalize(ro - vec3<f32>(0.0));

    // forward = rotate(forward, camera.rotationOffset.x, camera.rotationOffset.y, 0.0);
    ...
    // ro += vec3(2. * camera.posOffset.x, 2. * camera.posOffset.y, 0.0);
    // ro += camera.posOffset.x; // Horizontal pan
    // ro += camera.posOffset.y;     // Vertical pan
    var rd = normalize(uv.x * right + uv.y * up + forward);
    // var rd = normalize(vec3(uv, -1));
    // rd = rotate(rd, camera.rotationOffset.x, camera.rotationOffset.y, 0 );
